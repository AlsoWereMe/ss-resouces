...
# -------------------------- 1. 配置参数与基础函数 --------------------------
# 数据路径
TRAIN_MASK_CSV = "/mnt/data/FEPAI/project1/train_mask.csv"  # 训练集掩码CSV路径
TRAIN_IMG_DIR = "/mnt/data/FEPAI/project1/train/"  # 训练集图像文件夹路径
TEST_CSV = "/mnt/data/FEPAI/project1/test_a_samplesubmit.csv"  # 测试集提交模板CSV路径
TEST_IMG_DIR = "/mnt/data/FEPAI/project1/test/"  # 测试集图像文件夹路径
...
# 数据增强流程：尺寸调整→水平翻转→垂直翻转→随机旋转→位置偏移→颜色增强
...
class TTA_Transform:
    ...
    def apply_transform(self, img, transform_idx):
        """应用变换"""
        transform = self.transforms[transform_idx]
        augmented = transform(image=img)
        return augmented["image"]
    
    def reverse_transform(self, mask, transform_idx):
        """逆变换"""
        transform = self.transforms[transform_idx]
        ...
        if hasattr(transform, 'transforms'):
            for t in reversed(transform.transforms):
                mask = self._reverse_single_transform(mask, t)
        else:
            mask = self._reverse_single_transform(mask, transform)
            
        return mask
    
    def _reverse_single_transform(self, mask, transform):
        """单个变换的逆变换"""
        if isinstance(transform, A.HorizontalFlip):
            return cv2.flip(mask, 1)  # 水平翻转
        elif isinstance(transform, A.VerticalFlip):
            return cv2.flip(mask, 0)  # 垂直翻转
        elif isinstance(transform, A.Rotate):
            # 计算逆旋转角度
            angle = -transform.limit[0] if hasattr(transform.limit, '__len__') else -transform.limit
            h, w = mask.shape[:2]
            center = (w // 2, h // 2)
            M = cv2.getRotationMatrix2D(center, angle, 1.0)
            return cv2.warpAffine(mask, M, (w, h), flags=cv2.INTER_LINEAR)
        else:
            return mask  # NoOp或其他不需要逆变换的
...
def load_data():
    ...
    print(
        f"数据集总数: {dataset_size}, 训练集数量: {len(train_idx)}, 验证集数量: {len(valid_idx)}"
    )
...
def train_model(train_loader, valid_loader):
    ...
    for epoch in range(1, EPOCHES + 1):
        ...
        print(
            f"{epoch:^5d} |  {avg_train_loss:.4f}   |  {avg_valid_loss:.4f}   | {train_time:.2f}"
        )
...
# 模型测试 - 使用TTA
def predict_test_with_tta(model):
    """使用TTA预测测试集并生成提交文件"""
    # 读取测试集信息
    ...
    print(f"使用 {len(tta_transform)} 种TTA变换")

    # 预测并编码
    model.eval()
    submissions = []
    
    for idx, img_path in tqdm(enumerate(test_mask["name"].values), total=len(test_mask)):
        # 使用TTA进行预测
        pred_mask = predict_with_tta(model, img_path, tta_transform)
    ...
    # 保存提交文件
    pd.DataFrame(submissions).to_csv(
        SUBMISSION_SAVE_PATH, index=None, header=None, sep="\t"
    )
    print(f"\nTTA预测结果已保存至: {SUBMISSION_SAVE_PATH}")
    ...
    if submissions:
        ...
        if os.path.exists(sample_img_path):
            ...
            plt.subplot(122), plt.imshow(sample_mask, cmap="gray"), plt.title(
                "TTA预测掩码"
            )
            plt.savefig(PREDICTION_VIS_PATH)
            print(f"TTA预测样本可视化已保存至: {PREDICTION_VIS_PATH}")
        else:
