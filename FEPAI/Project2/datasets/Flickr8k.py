...
class Flickr8kDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple:
        ...
        # 1. 将原始的 caption 字符串编码为 token ID 列表。
        ...
        sos_id = self.tokenizer.get_sos_token_id
        eos_id = self.tokenizer.get_eos_token_id
        model_input_sequence = [sos_id] + caption_tokens + [eos_id]
        
        # 3. 将最终的 ID 列表转换为 PyTorch 的 LongTensor。
        #    - 这是模型期望的输入格式。
        caption_output = torch.LongTensor(model_input_sequence)
